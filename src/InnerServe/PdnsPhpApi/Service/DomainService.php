...
class DomainService extends Service {
	...
	public function create( $domain, $type = 'MASTER', $master = null ) {
		...
		$stmt->execute( array( 'name' => strtolower( $domain ), 'type' => $type, 'master' => $master ) );
	...
	public function exists( $domain ) {
		...
		$stmt->execute( array( 'domain' => strtolower( $domain ) ) );
	...
	public function get( $domain ) {
		...
		$stmt = $this->getPdo()->prepare( "SELECT `id`, `name`, `type`, `content`, `ttl`, `prio`, `change_date` FROM records WHERE domain_id = :domain_id" );
